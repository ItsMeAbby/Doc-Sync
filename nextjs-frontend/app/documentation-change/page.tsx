...
export default function DocumentationChangePage() {
  ...
  const [response, setResponse] = useState<EditDocumentationResponse | null>(
    null,
  );
  ...
  const handleViewDocument = () => {
    if (documentDetails && documentId) {
      ...
      sessionStorage.setItem(
        "selectedLanguage",
        documentDetails.language || "en",
      );
      sessionStorage.setItem(
        "selectedTab",
        documentDetails.is_api_ref ? "api_references" : "documentation",
      );
  ...
  const changesToApply = useMemo(() => {
    return filteredChanges.filter((change) => {
      ...
      const isDelete =
        "document_id" in change &&
        "version" in change &&
        !("changes" in change) &&
        !("markdown_content_en" in change);
      let changeId = "";
  ...
  const getChangeType = (): "edit" | "create" | "delete" | "mixed" => {
    ...
    const hasDeletes = changesToApply.some(
      (c) =>
        "document_id" in c &&
        "version" in c &&
        !("changes" in c) &&
        !("markdown_content_en" in c),
    );
  ...
  const handleApplySingle = async (
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/edit/update_documentation`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        throw new Error(
          `Failed to apply change: ${res.status} ${res.statusText}`,
        );
      }
      ...
      if (result.failed > 0 && result.failed_items) {
        ...
        if (type === "edit") {
          ...
          requestedChangeId =
            createChangeIds.get(change as GeneratedDocument) || "";
        } else {
          requestedChangeId =
            deleteChangeIds.get(change as DocumentToDelete) || "";
        }
        ...
        if (currentResponse) {
          ...
          if (type === "edit") {
            const isFailed = result.failed_items.edit?.some(
              (failed) => failed.document_id === requestedChangeId,
            );
            if (!isFailed && newResponse.edit) {
              // Success: remove from response
              newResponse.edit = newResponse.edit.filter(
                (edit) => edit.document_id !== requestedChangeId,
              );
            }
          } else if (type === "create") {
        ...
        if (result.failed_items?.edit && type === "edit") {
          ...
          if (isFailed) {
            ...
            setDocumentContents((prev) => {
              ...
              const failedEdit = result.failed_items?.edit?.find(
                (failed) => failed.document_id === requestedChangeId,
              );
  ...
  const handleIgnoreSingle = (changeId: string) => {
    ...
    if (changeId.startsWith("create-")) {
      ...
      if (newResponse.edit) {
        newResponse.edit = newResponse.edit.filter(
          (edit: DocumentEdit) => edit.document_id !== changeId,
        );
      }
    }
  ...
  const handleConfirmApply = async () => {
    ...
    try {
      if (confirmAction === "ignore") {
        ...
        const editsToApply = changesToApply.filter(
          (c) => "changes" in c,
        ) as DocumentEdit[];
        const createstoApply = changesToApply.filter(
          (c) => "markdown_content_en" in c,
        ) as GeneratedDocument[];
        const deletesToApply = changesToApply.filter(
          (c) =>
            "document_id" in c &&
            "version" in c &&
            !("changes" in c) &&
        ...
        const editsWithOriginal: DocumentEditWithOriginal[] = editsToApply.map(
          (edit) => ({
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to apply changes: ${res.status} ${res.statusText}`,
          );
        }
        ...
        if (result.failed > 0 && result.failed_items) {
          ...
          if (currentResponse) {
            ...
            const requestedDeleteIds = new Set(
              deletesToApply.map(
                (deleteChange) => deleteChangeIds.get(deleteChange) || "",
              ),
            );
            ...
            if (newResponse.edit) {
              newResponse.edit = newResponse.edit.filter((edit) => {
                ...
                const isFailed = result.failed_items?.edit?.some(
                  (failed) => failed.document_id === edit.document_id,
                );
    ...
    <div className="container mx-auto px-2 sm:px-4 py-4">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Describe what needs to be updated in the documentation and the system
      ...
      <Card className="p-4 sm:p-6 mb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Enter a natural language description of what changed or what needs to
          be updated in the documentation. Example: "We don't support agents
          as_tool anymore, other agents should only be invoked via handoff"
        </p>
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300">
                    ...
                    {documentDetails.language || "N/A"}
                  </p>
                  <p className="text-sm text-blue-700 dark:text-blue-300">
                    <strong>Type:</strong>{" "}
                    {documentDetails.is_api_ref
                      ? "API Reference"
                      : "Documentation"}
                  </p>
                </div>
                <div className="flex justify-between items-center mt-3">
                  <p className="text-xs text-blue-600 dark:text-blue-400">
                    This update request will be specifically analyzed for this
                    document.
                  </p>
                  <Button
                    variant="outline"
          ...
          <ScrollArea className="h-[600px] pr-4">
            <div className="space-y-4 pt-4">
              {filteredChanges.map((change, index) => {
                ...
                const originalContentDict = isEdit
                  ? documentContents.get(
                      (change as DocumentEdit).document_id,
                    ) || {
                      markdown_content: "",
                    }
                  : {
                ...
                return (
                  <DocumentChangeCard
                    ...
                    onSelectionChange={(selected) =>
                      handleSelectionChange(changeId, selected)
                    }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action will {confirmAction === "ignore" ? "ignore" : "apply"}{" "}
              {getDocumentCount()} change{getDocumentCount() !== 1 ? "s" : ""}.
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={applyingChanges}
              className={
                confirmAction === "ignore" ? "bg-red-600 hover:bg-red-700" : ""
              }
            >
              {applyingChanges ? (
                <>
                  <Spinner size="sm" className="mr-2" />
                  {confirmAction === "ignore" ? "Ignoring" : "Applying"}{" "}
                  changes...
                </>
              ) : (
