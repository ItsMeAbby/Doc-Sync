...
export const documentVersionsApi = {
  ...
  async getDocumentVersion(
    ...
    const apiBaseUrl =
      process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8000";
    const response = await fetch(
      `${apiBaseUrl}/api/documents/${documentId}/versions/${versionId}`,
    );

    if (!response.ok) {
      const response = await fetch(
        `${apiBaseUrl}/api/documents/${documentId}/versions/latest`,
      );
      if (!response.ok) {
        throw new Error(
          `Failed to fetch document version: ${response.statusText}`,
        );
      }
    }
