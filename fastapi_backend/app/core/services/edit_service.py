...
class EditService:
    ...
    async def edit_documentation(
        self, edit_request: EditDocumentationRequest
    ) -> EditDocumentationResponse:
        """Edit documentation based on a query"""
        editor = MainEditor(
            query=edit_request.query, document_id=edit_request.document_id
        )
    ...
    async def process_single_edit(
        self, edit: DocumentEditWithOriginal
        ...
        try:
            ...
            try:
                ...
                if document.get("is_deleted", False):
                    return (
                        False,
                        f"Document {edit.document_id} is deleted and cannot be edited",
                    )
            ...
            logger.error(
                f"Error processing edit for document {edit.document_id}: {str(e)}"
            )
            return False, str(e)

    async def process_single_create(
        self, doc: GeneratedDocument
    ) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            for language, markdown_content in [
                ("en", doc.markdown_content_en),
                ("ja", doc.markdown_content_ja),
            ]:
    ...
    async def process_single_delete(
        ...
        try:
            ...
            logger.error(
                f"Error processing delete for document {doc.document_id}: {str(e)}"
            )
            return False, str(e)
    ...
    async def update_documentation(
        ...
        if (
            not change_request.edit
            and not change_request.create
            and not change_request.delete
        ):
        ...
        if edit_items:
            ...
            for i, result in enumerate(edit_results):
                if isinstance(result, Exception):
                    ...
                    errors.append(
                        ProcessingError(
                            error_message=str(result), error_type=type(result).__name__
                        )
                    )
